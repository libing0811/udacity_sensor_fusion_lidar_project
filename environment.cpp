...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloudPointer=lidar->scan();
    //renderRays(viewer, lidar->position,cloudPointer );
    //renderPointCloud(viewer, cloudPointer, "test point cloud");
      
    // TODO:: Create point processor
    ProcessPointClouds<pcl::PointXYZ> processor;
    ProcessPointClouds<pcl::PointXYZ> * processorPointer = new ProcessPointClouds<pcl::PointXYZ>();
  
    std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr, pcl::PointCloud<pcl::PointXYZ>::Ptr> segmentCloud = processorPointer->SegmentPlane(cloudPointer, 1000, 0.3);
    //renderPointCloud(viewer,segmentCloud.first,"obstCloud",Color(1,0,0));
    //renderPointCloud(viewer,segmentCloud.second,"planeCloud",Color(0,1,0));
  
    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> cloudClusters = processorPointer->Clustering(segmentCloud.first, 2, 3, 30);
    ...
    for(pcl::PointCloud<pcl::PointXYZ>::Ptr cluster : cloudClusters)
    {
          ...
          processorPointer->numPoints(cluster);
          renderPointCloud(viewer,cluster,"obstCloud"+std::to_string(clusterId),colors[clusterId]);
      		
          Box box = processorPointer->BoundingBox(cluster);
...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer, ProcessPointClouds<pcl::PointXYZI>* pointProcessorI, const pcl::PointCloud<pcl::PointXYZI>::Ptr & inputCloud)
{
    ...
    for(pcl::PointCloud<pcl::PointXYZI>::Ptr cluster : cloudClusters)
    {
          ...
          //Finding Bounding Box of the cluster
